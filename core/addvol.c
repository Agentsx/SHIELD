...
int __addvol_check_limit(add_vol_req_t *req)
{
	...
	if (ret) {
		log_warn("WARNING: [%s][%d] Add vol get etf[%s] trade vol failed.\n", __FL__, req->instrument_id);
		SET_RESULT(SO_BAD);
		return FALSE;
	}
    ...
    if (ret) {
		log_warn("WARNING: [%s][%d] Add vol get etf[%s] trade list.\n", __FL__, req->instrument_id);
		SET_RESULT(SO_BAD);
        return FALSE ;
    }
...
static int __addvol_check_sge_instruction(const char *instruction_id)
{
	...
	if (ret) {
		log_warn("WARNING: [%s][%d] Add vol get sge instructions failed.\n", __FL__);
        SET_RESULT(SO_BAD);
        goto ERROR;
	}
	...
	if (ret) {
		log_warn("WARNING: [%s][%d] Add vol sge instructions already handled.\n", __FL__);
...
static int __addvol_req_check(add_vol_req_t *req)
{
	...
	ret=__addvol_check_trade_time(); // TODO:
	if (ret) {
		log_warn("WARNING: [%s][%d] Add vol check trade time failed.\n", __FL__);
		return FALSE;
	}
	
	ret = __addvol_check_sge_instruction(req->instruction_id);
	if (ret) {
		log_warn("WARNING: [%s][%d] Add vol check sge instruction failed.\n", __FL__);
		return FALSE;
	}
	...
	if (ret) {
		log_warn("WARNING: [%s][%d] Add vol check client failed.\n", __FL__);
		return FALSE;
	}
...
{
	log_notice("TRACE: [%s][%d] add vol handler called.\n", __FL__);
	...
	if (ret) {
		log_warn("WARNING: [%s][%d] Add vol check failed.\n", __FL__);
		goto AFTER;
	}
